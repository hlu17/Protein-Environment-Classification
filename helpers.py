...
def official_data_split():
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.3, random_state=1, stratify=Y.iloc[:,1])
    
    return X_train.reset_index(drop=True), X_test.reset_index(drop=True), Y_train.reset_index(drop=True), Y_test.reset_index(drop=True)


#-------- DATA SETUP for gold categories
def official_data_split_gold():
    labels = pd.read_csv('data//metadata_by_jgi_spid.tsv', sep='\t', header=0)
    features = pd.read_csv('data//features.csv',header=None)
    columns = pd.read_csv('data//feature_column_names.tsv', sep='\t', header=None)
    ...
    # assign features and labels
    X = features.loc[:, 'PF00001.19':'PF17225.1']
    Y = labels[['ECOSYSTEM', 'ECOSYSTEM_CATEGORY', 'ECOSYSTEM_TYPE','HABITAT']]
    
    # Split the training and test dataset by 70/30 ratio, stratifies by class labels
    X_train, X_test, Y_train_gold, Y_test_gold = train_test_split(X, Y, test_size=0.3, random_state=1,stratify=Y.iloc[:,1])
...
#------------QUERYING DATABASE GOLD

def get_training_observations_gold():
    print(f"Getting all training observations from '{DATABASE_NAME}'...")
    conn = sqlite3.connect('..//data//' + DATABASE_NAME)
    
    x_train_transposed_gold = pd.read_sql_query("SELECT * FROM x_train_gold", con = conn)
    
    conn.commit()
    conn.close()
    
    x_train_transposed_gold.set_index('index', inplace=True)
    x_train_gold = x_train_transposed_gold.T
    x_train_normalized_gold = get_protein_proportions(x_train_gold)
    
    return x_train_normalized_gold

def get_training_labels_gold():
    print(f"Getting all training labels from '{DATABASE_NAME}'...")
    conn = sqlite3.connect('..//data//' + DATABASE_NAME)
...
def get_test_observations_gold():
    print(f"Getting all test observations from '{DATABASE_NAME}'...")
    conn = sqlite3.connect('..//data//' + DATABASE_NAME)
